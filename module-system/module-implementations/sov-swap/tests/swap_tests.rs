use helpers::add_liquidity_helper;
...
use crate::helpers::{create_pool_helper, create_token_helper};
...
fn create_pool() {
    ...
    let token_a = create_token_helper(
        sov_bank::Bank::new(),
        &mut working_set,
        &C::new(generate_address("")),
        "TokenA".to_owned(),
        100,
        generate_address(""),
        vec![generate_address("")],
    ).unwrap();

    let token_b = create_token_helper(
        sov_bank::Bank::new(),
        &mut working_set,
        &C::new(generate_address("")),
        "TokenB".to_owned(),
...
fn add_liquidity() {
    ...
    let token_a = create_token_helper(
        ...
        vec![private_key.default_address().clone()],
    ).unwrap();
    ...
    // Create Pool
    let (pool_id, liquidity_token) = create_pool_helper(
        swap.clone(),
        &mut working_set,
        &C::new(private_key.default_address().clone()),
        token_a.clone(),
    ...
    assert_eq!(
        ...
        Event::new(
            ...
            &format!(
                ...
                send_token = token_a.clone(),
                send_amount = 100,
                receive_token = token_b.clone(),
                receive_amount = 200,
            ),
        )
    );
